...
public class TGrafoNoDirigido<T> extends TGrafoDirigido<T> implements IGrafoNoDirigido<T> {
    ...
    /**
     * Encuentra todos los puntos de articulación en este grafo no dirigido.
     * Algoritmo de Tarjan.
     * @return Conjunto de etiquetas de los vértices que son punto de articulación.
     */
    public Set<Comparable> puntosArticulacion() {
        Map<Comparable, Boolean> visited = new HashMap<>();
        Map<Comparable, Integer> disc = new HashMap<>();
        Map<Comparable, Integer> low = new HashMap<>();
        Map<Comparable, Comparable> parent = new HashMap<>();
        Set<Comparable> artPoints = new HashSet<>();
        int time = 0;

        // Inicializar estructuras
        for (Comparable u : getVertices().keySet()) {
            visited.put(u, false);
            parent.put(u, null);
        }

        // Lanzar DFS desde cada componente
        for (Comparable u : getVertices().keySet()) {
            if (!visited.get(u)) {
                time = dfsArt(u, visited, disc, low, parent, artPoints, time);
            }
        }
        return artPoints;
    }
    /**
     * DFS recursivo que actualiza disc[], low[] y detecta puntos de articulación.
     * @return el nuevo valor de time tras procesar el subárbol.
     */
    private int dfsArt(Comparable u,
                       Map<Comparable, Boolean> visited,
                       Map<Comparable, Integer> disc,
                       Map<Comparable, Integer> low,
                       Map<Comparable, Comparable> parent,
                       Set<Comparable> artPoints,
                       int time){
        ...
        IVertice<T> verticeU = getVertices().get(u);
        for (IAdyacencia<T> ady : verticeU.getAdyacentes()){
            ...
            if (!visited.get(v)) {
                ...
                time = dfsArt(v, visited, disc, low, parent, artPoints, time);

                // Actualizo low[u]
                low.put(u, Math.min(low.get(u), low.get(v)));

                // Caso raiz. Recordemos que un Articulation point es si tiena mas de dos hijos la raiz.
