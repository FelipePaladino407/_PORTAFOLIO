...
public class TGrafoDirigido<T> implements IGrafoDirigido<T> {
    ...
    public LinkedList<Tarea> ordenParcial(){
        LinkedList<Tarea> orden = new LinkedList<>();
        for (IVertice<T> vertV : vertices.values()) {
            // Reiniciamos estados.
            vertV.setVisitado(false);
        }
        for (IVertice<T> vertV : vertices.values()) {
            if (!vertV.getVisitado()) {
                ((TVertice<T>) vertV).ordenTopologico(orden); // La dichosa recursion.
            }
        }
    ...
    /**
     * UT7_PD7 - Metodo para listar las tareas,
     * que muestra por consola la secuencia de estas.
     *
    public void listarTareas(LinkedList<Tarea> tareas){
        for (Tarea tarea : tareas) {
            System.out.println(tarea.getCodigo());
        }
    }
**/

    /**
     * UT7_PD8 - Metodo para clasificar tipos de arcos.
     * Se basa en una tecnica de colores boba.
     * @param etiquetaOrigen
     */
    public void clasificarArcos(Comparable etiquetaOrigen) {

        for (IVertice<T> vertV : vertices.values()) {
            ((TVertice<T>) vertV).reiniciarArcos();
        }

        // Los 4 tipos de arcos. Son coleccioes donde acumular los vertics.
        TAristas arbol = new TAristas();
        TAristas retroceso = new TAristas();
        TAristas avance = new TAristas();
        TAristas cruzados = new TAristas();

        Tiempo reloj = new Tiempo();

        // Arranco a buscar desde el origen, y utilizo el metodo de TVertice:
        TVertice<T> inicio = (TVertice<T>) buscarVertice(etiquetaOrigen);
        if (inicio != null && inicio.esBlanco()) {
            inicio.clasificarArcos(arbol, retroceso, avance, cruzados, reloj);
        }

        // Vamos por las componentes no conectadas:
        for (IVertice<T> vertV : vertices.values()) {
            TVertice<T> tv = (TVertice<T>) vertV;
            if (tv.esBlanco()){
                tv.clasificarArcos(arbol, retroceso, avance, cruzados, reloj);
            }
        }

        // Mostramos resultetes:
        System.out.println("Árbol:\n"      + arbol.imprimirEtiquetas());
        System.out.println("Retroceso:\n"  + retroceso.imprimirEtiquetas());
        System.out.println("Avance:\n"     + avance.imprimirEtiquetas());
        System.out.println("Cruzados:\n"   + cruzados.imprimirEtiquetas());
