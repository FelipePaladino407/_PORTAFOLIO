...
public class TVertice<T> implements IVertice<T> {
     ...
     * UT7_PD7
     *
    public void ordenTopologico(LinkedList<Tarea> listaOrden) {
        this.setVisitado(true);

        for (IAdyacencia<T> adyacencia : this.getAdyacentes()) {
            IVertice<T> destino = adyacencia.getDestino();
            if (!destino.getVisitado()) {
                ((TVertice<T>) destino).ordenTopologico(listaOrden);
            }
        }
        listaOrden.addFirst((Tarea) this.getDatos());
    }
    ...
    /**
     * UT7_PD8
     * Cual es la idea de los colores?
     * BLANCO: El vertice aun no se ha descubierto.
     * GRIS: Lo estamo' explorando (en realidad ya lo descubriste, pero aun no procesaste
     * todas sus aristas).
     * NEGRO: Ya exploraste todas sus adyacentes y cerraste el vertice.
     */

    public void clasificarArcos(
            TAristas arcoArbol,
            TAristas arcoRetroceso,
            TAristas arcoAvance,
            TAristas arcoCruzado,
            Tiempo reloj)
    {

        this.color = Color.GRIS;
        this.discoveryTime = ++reloj.time;

        // Recorro cada adyacencia:
        for (IAdyacencia<T> adyacente : adyacentes) {
            TVertice<T> vertice = (TVertice<T>) adyacente.getDestino();
            switch (vertice.color){
                case BLANCO:
                    arcoArbol.add( toArista(adyacente) );
                    vertice.clasificarArcos(arcoArbol, arcoRetroceso, arcoAvance, arcoCruzado, reloj);
                    break;
                case GRIS:
                    arcoRetroceso.add( toArista(adyacente) );
                    break;
                case NEGRO:
                    if (this.discoveryTime < vertice.discoveryTime)
                        arcoAvance.add( toArista(adyacente) );
                    else
                        arcoCruzado.add( toArista(adyacente) );
                    break;


            }
        }
        this.color = Color.NEGRO;  // <_>
        this.finishTime = ++reloj.time;


    }

    /**
     * Convierte la adyacencia "adyacente" en un IArista con origen=this.etiqueta.
     */
    private IArista toArista(IAdyacencia<T> ad) {
        return new TArista(
                this.getEtiqueta(),              // etiqueta de origen
                ad.getDestino().getEtiqueta(),   // etiqueta de destino
                ad.getCosto()                    // mismo costo
        );
    }

    public void reiniciarArcos(){
